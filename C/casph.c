...
// Completion status of each nodes
uint8_t* complete;

// Pruned status at each depth
uint32_t *pruned;

// Number of 64-bit words in reaching vector
const uint64_t vectorSize = 8;
...
void readGraph(const char* filename)
{
  ...
  complete = (uint8_t*) malloc(numNodes);
  pruned = (uint32_t*) calloc(numNodes, sizeof(uint32_t));
...
uint64_t ssp(uint32_t from, uint32_t to)
{
  ...
  // Initialise completion status of each node
  memset(complete, 0, numNodes);
  ...
  while (! done) {
    // For each node
    for (int i = 0; i < numNodes; i++) {
      if (complete[i]) {pruned[dist]++; continue;}
      // For each neighbour
    ...
    for (int i = 0; i < numNodes; i++) {
      ...
      if (! complete[i]) {
        ...
        if (n != nodesInVector) done = 0; else complete[i] = 1;
      }
    }
